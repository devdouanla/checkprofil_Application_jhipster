...
/**
 * REST controller for managing {@link com.devdouanla.domain.Resultat}.
 */
@RestController
...
public class ResultatResource {
    ...
    public ResponseEntity<List<ResultatDTO>> getAllResultats(
        ...
        if ("session-is-null".equals(filter)) {
            LOG.debug("REST request to get all Resultats where session is null");
            return new ResponseEntity<>(resultatService.findAllWhereSessionIsNull(), HttpStatus.OK);
        }
