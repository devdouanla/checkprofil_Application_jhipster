...
public interface ManagerRepository extends JpaRepository<Manager, Long> {
    ...
    @Query(
        value = "select manager from Manager manager left join fetch manager.user",
        countQuery = "select count(manager) from Manager manager"
    )
    Page<Manager> findAllWithToOneRelationships(Pageable pageable);

    @Query("select manager from Manager manager left join fetch manager.user")
